...
// Sequence-position Variables. 
int firstPosition[4];
int lastPosition[4];
int currentPosition[4];
int tempPosition[4];

// Variables for the color detected.
...
// Function to move the robot with the encoder and direction 
...
  array[0] = sensor.readRangeContinuousMillimeters();  // Read the laser sensor distance.
  array[1] = USRight.read();  // Read the right ultrasonic sensor distance.
  array[2] = USLeft.read();   // Read the left ultrasonic sensor distance
}
...
        break;
    }
    grade = map(dif, 0, 100, 0, 100);  // Map the difference to a grade value.
...
  center();  // Center again the direction. 
  stop();
}
...
  while (!finaliza) {
    int closestIndex = -1;
    pixy.ccc.getBlocks();  // Obtain the objects detected block by the Pixy
    readDistance(currentPosition);  // Read the current distances.
